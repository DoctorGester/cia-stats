/**
 * Created by kartemov on 07.10.2016.
 */
"use strict";
...
var KeyValues = function(key, value) {
    this.key = key || "";
    this.value = value || "";

    this.load = function(kvString, options) {
        options = options || {};
        ...
        while (!finished) {
            var results = getNextToken(kvString, options);
    ...
    this.recursiveLoad = function(kvString, options) {
        var finished = false;
        var extracted = [];

        var opening = getNextToken(kvString, options);
        ...
        var closing = getNextToken(kvString, options);
    ...
    this.recursiveSave = function(depth, options) {
        var kvString = '';

        if (Array.isArray(this.value)) {
            var subKeys = this.value.slice();
            ...
            if (options.sortKeys) {
                subKeys.sort(function(a, b) {
                    ...
                    var firstKey = a.key.toLowerCase();
                    var secondKey = b.key.toLowerCase();
    ...
    this.addSubKey = function(key, value) {
        if (!Array.isArray(this.value)) {
